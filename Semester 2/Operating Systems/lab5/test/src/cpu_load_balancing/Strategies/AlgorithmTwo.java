...
public class AlgorithmTwo implements Strategy{
    ...
    public void onTaskAssigned(CPU assignedTo, Task task) {
        if(assignedTo.getLoad() < Config.p) {
            return;
        }
        int[] cpusToQuery = broker.generateRandomCpuSequence(assignedTo, broker.getCpuCount() - 1);
