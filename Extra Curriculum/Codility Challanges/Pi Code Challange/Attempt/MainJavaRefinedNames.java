...
public class MainJavaRefinedNames {
    ...
    public static int solution(String initialString, String finalString) {
        ...
        int initStringLength = initialString.length();
        for (int i = 0; i < initStringLength; ++i) {

            // 'a' is a single char value
            int initialChar = initialString.charAt(i) - 'a';
            int finalChar = finalString.charAt(i) - 'a';

            // Mark all the valid transitions between characters in the input strings
        ...
        for (int i = 0; i < 26; ++i) {
            for (int j = 0; j < 26; ++j) {
                if (allowedTransitions[i][j]) {
                    remainingChars = remainingChars | (1 << i);
                    break;
                }
            }
        }
        ...
        int[] minOperations = new int[] { initStringLength };
            ...
            for (int i = currentChar + 1; i < 26; ++i) {
                if (allowedTransitions[i][currentChar]) {
                    if ((remainingChars & (1 << i)) > 0) {
                        // Set the bit corresponding to each character that has a valid transition from it
                        // bitwise OR 
                        temp = temp | (1 << i);
                        ++numTransitions;
                    }
